...
It also allows for signtaure entries that represent runtime information,
such as current iteration index using \Code{\Ci{WorkId}}.
...
\end{figure}
%
The \Code{\Ci{dax::cont::Schedule}} object is a variadic functor
whose arguments are the worklet followed by the worklet's arguments.
In Figure~\ref{fig:InvokeSine} we see using the \Code{\Ci{dax::cont::Schedule}} object two
execute the worklet from Figure~\ref{fig:Sine} twice.
